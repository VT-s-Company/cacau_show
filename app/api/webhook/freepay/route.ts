    ...
    if (data.status === "CONFIRMED" || data.status === "PAID") {
      ...
      if (stored) {
        const checkoutData: PaymentEmailData = {
          ...
          customerCpf: stored.customerCpf || undefined,
          customerPhone: stored.customerPhone || undefined,
          amount: stored.amount,
          items: stored.items ? JSON.parse(stored.items) : [],
          shippingAddress: {
            street: stored.street || "",
            number: stored.streetNumber || "",
            complement: stored.complement || undefined,
            district: stored.district || "",
            city: stored.city || "",
            state: stored.state || "",
            zip_code: stored.zipCode || undefined,
          },
